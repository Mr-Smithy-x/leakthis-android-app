...
public class HtmlParser implements Html.TagHandler, ContentHandler
{
    ...
    public static HtmlParser buildSpannedText(TagHandler handler)
    {
    ...
    public static Spanned buildSpannedText(String html, Html.ImageGetter imageGetter, TagHandler handler)
    {
        // add a tag at the start that is not handled by default,
        // allowing custom tag handler to replace xmlReader contentHandler
        return Html.fromHtml("<inject/>" + html, Html.FROM_HTML_MODE_COMPACT, imageGetter, new HtmlParser(handler));
    }
